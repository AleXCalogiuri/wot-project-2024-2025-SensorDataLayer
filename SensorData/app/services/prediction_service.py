...
import pandas as pd
from ..senders.RabbitMQSender import RabbitMQSender
RABBITMQ_HOST = 'localhost'
RABBITMQ_QUEUE_NAME = 'road_prediction_queue'
...
class PredictionService:
    ...
    def send_prediction(predicition_request_dto: PredictRequestDTO):
        try:
            ...
            via_ricevuta = predicition_request_dto.via
            ...
            print(f"PREDICTION SERVICE response for HTTP and RabbitMQ: {response}")

            try:
                print("Tentativo di invio messaggio a RabbitMQ...")
                rabbitmq_sender = RabbitMQSender(host=RABBITMQ_HOST)
                rabbitmq_sender.publish_message(
                    queue_name=RABBITMQ_QUEUE_NAME,
                    message_dict=response
                )
            except Exception as e:

                print(f"ATTENZIONE: Fallito l'invio del messaggio a RabbitMQ. L'errore è: {e}")
            return response, 200
